...
const AddRoleModal = ({ load, closeModal }) => {
    const [loadPermissions, { data, isLoading, isError, isSuccess, error }] = adminApiSlice.useGetAllPermissionsMutation();
    const [createRole, { data: createdData, isLoading: createLoading, isError: createIsError, isSuccess: createIsSuccess, error: createError }] = adminApiSlice.useCreateRoleMutation();
    ...
    const handleCreate = () => {
        if (roleName && checkedPermissions.length > 0) {
            createRole({ name: roleName, permissions: checkedPermissions })
        }
    }
    useEffect(() => {
        if (createIsSuccess) {
        ...
        if (createIsError) {
            ...
            if ([422, 401].includes(createError.status)) {
                    ...
                    <AnimatePresence>
                        <motion.div transition={{ duration: 0.2, damping: 0, type: 'keyframes' }} initial={{ scale: 0.99, y: '-100px' }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.99, y: '-100px' }} className="w-full inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                            <div className="bg-white md:w-[500px] px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div className="sm:flex sm:items-start">
                                    ...
                                    <div className="w-full  sm:mt-0 sm:ml-4 sm:text-left">
                                        <h3 className="text-xl leading-6 font-medium text-gray-900" id="modal-headline">Create new role</h3>
                                        <div className="mt-3">
                            ...
                            <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                <button onClick={() => handleCreate()} type="button" className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-500 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">{createLoading ? 'Saving' : 'Save'}</button>
